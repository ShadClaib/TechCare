...
onMounted(async () => {
  let username = prompt("Enter username", "coalition");
      ...
      <WidgetWrapper :title="'Patients'">
        ...
        <template #content>
          <div>
            ...
            <li v-for="(profile, index) in profiles" v-bind:key="index" :class="{'cursor-pointer':true, 'bg-[--activestate_bg_2]':profileSelected.name === profile.name}" @click="profileSelected = profile">
              <ProfileBadge :photo="profile.profile_picture" :photo2x="profile.profile_picture" :name="profile.name" :title="`${profile.gender}, ${profile.age}`" class="w-full" :after="true">
                <template #after>
        ...
        <template #content>
          <div class="m-5 flex flex-col space-y-8">
            <div class="flex space-x-8 bg-[#F4F0FE] p-5 rounded-xl">
              <div class="basis-3/4">
                <div class="flex items-center justify-between">
                  ...
                  <div>Last 6 months <img src="@/assets/images/expand_more_FILL0_wght300_GRAD0_opsz24.svg" class="inline-block ms-4" /></div>
                </div>
              ...
              <div class="basis-1/4">
                <div>
                  <div class="inline-flex items-center space-x-2"><i class="w-[.875rem] h-[.875rem] bg-[#E66FD2] border border-white rounded-full inline-block">&nbsp;</i> <span class="font-medium">Systolic</span></div>
                ...
                <div>
                  <div class="inline-flex items-center space-x-2"><i class="w-[.875rem] h-[.875rem] bg-[#8C6FE6] border border-white rounded-full inline-block">&nbsp;</i> <span class="font-medium">Diastolic</span></div>
            ...
            <Grid class="grid-cols-3 gap-5">
              <StatBox :icon="respiratoryRateImage" :label="'Respiratory Rate'" :stat="`${dataPointSelected?.respiratory_rate.value} bpm`" class="bg-[#E0F3FA]">
                <template #after>
                  {{dataPointSelected?.respiratory_rate.levels}}
                </template>
              </StatBox>
              <StatBox :icon="temperatureImage" :label="'Temperature'" :stat="`${dataPointSelected?.temperature.value}°F`" class="bg-[#FFE6E9]">
                <template #after>
                  {{dataPointSelected?.temperature.levels}}
                </template>
              </StatBox>
              <StatBox :icon="HeartBPMImage" :label="'Heart Rate'" :stat="`${dataPointSelected?.heart_rate.value} bpm`" class="bg-[#FFE6F1]">
                <template #after>
        ...
        <template #content>
          ...
          <div class="table table-fixed border-collapse w-full">
            <div class="table-header-group">
              <div class="table-row [&>.table-cell]:font-semibold [&>.table-cell]:bg-[#F6F7F8] [&>.table-cell]:px-5 [&>.table-cell]:py-4 [&>.table-cell:first-child]:rounded-l-full [&>.table-cell:last-child]:rounded-r-full">
                <div class="table-cell">Problem/Diagnosis</div>
                <div class="table-cell">Description</div>
                <div class="table-cell">Status</div>
              </div>
            </div>
            <div class="table-row-group [&>.table-row>.table-cell]:px-5 [&>.table-row>.table-cell]:py-4 [&>.table-row>.table-cell]:border-[#F6F7F8] [&>.table-row:not(:last-child)>.table-cell]:border-b">
        ...
        <template #content>
          <header class="px-5 pt-5">
            <picture>
              ...
              <img :src="profileSelected?.profile_picture" :alt="profileSelected?.name" class="w-[12.5rem] block mx-auto mt-3" />
            </picture>
          ...
          <div class="px-5 pb-5">
            <ul class="[&>li]:my-6">
              ...
              <li>
                <IconBullet :icon="PhoneIcon" :label="'Emergency Contacts'" :text="profileSelected?.emergency_contact" />
              </li>
              <li>
                <IconBullet :icon="InsuranceIcon" :label="'Insurance Provider'" :text="profileSelected?.insurance_type" />
              </li>
            </ul>
            <button class="text-center font-semibold block px-12 py-2.5 rounded-full bg-[--activestate_bg_1] mx-auto">Show All Information</button>
          </div>
        </template>
      </WidgetWrapper>
